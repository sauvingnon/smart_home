...
export function useChatListAnchor(
  ...
  /**
   * Прыжок к конкретному сообщению: из баннера закрепа и из цитаты в ответе.
   *
   * Раньше это делал сам ChatPage через node.scrollIntoView, в обход хука, и
   * ломалось сразу двумя способами.
   *
   * Первое: режим залипания оставался включённым. Снимает его только жест
   * пользователя, а тап по цитате жеста не создаёт — touchmove при нём не
   * приходит, окно жеста не открывается, и handleScroll выходит на первой же
   * проверке. То есть юзер прыгал к старому сообщению, оставаясь формально "у
   * низа", и первое же пришедшее сообщение утаскивало его обратно вниз — с
   * того самого места, куда он только что прыгнул.
   *
   * Второе: scrollIntoView запускает свою smooth-анимацию, ничего про неё не
   * сообщая, поэтому pin() имел полное право написать scrollTop прямо поверх
   * неё и оборвать на полпути.
   *
   * Здесь прыжок объявлен явным переходом (как кнопка "вниз", только цель
   * другая): низ отпускаем, окно smooth-скролла заводим.
   */
  const scrollToNode = useCallback((node: HTMLElement) => {
    const el = containerRef.current;
    if (!el) return;

    // Прыгнуть можно и к самому последнему сообщению — закреп нередко им и
    // является. Это тот же переход вниз, и держать низ дальше снова наше дело;
    // во всех остальных случаях юзер ушёл в историю, и низ не наш.
    //
    // Решаем это здесь и сразу, по положению цели в ленте, а не проверкой
    // "доехали ли до низа" после анимации. Такая проверка требует знать, когда
    // scrollIntoView закончил, а он об этом не сообщает: по таймеру она
    // промахивается тем вернее, чем дальше прыжок (на длинной дистанции
    // анимация идёт дольше любого разумного окна), а событий scroll вне окна
    // жеста мы намеренно не слушаем.
    const all = el.querySelectorAll<HTMLElement>('[data-seq]');
    setStick(all.length > 0 && all[all.length - 1] === node);

    smoothUntilRef.current = Date.now() + SMOOTH_SCROLL_MS;

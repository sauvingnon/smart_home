...
export const ChatPage: React.FC = () => {
  ...
  const openGalleryPicker = () => {
    ...
    // Уходим из режима "печатают" явно и до открытия пикера. Клавиатуру всё
    // равно убирает сам пикер, на уровне ОС, — удержать её нельзя, а вот
    // рассинхрон с ней получить очень легко. Именно это и выходило, когда тап
    // по скрепке гасил дефолт pointerdown ради сохранения фокуса: клавиатуры
    // на экране нет, а inputFocused всё ещё true — таб-бар остаётся скрытым
    // (useHideNavBar), инпут-бар сидит в позиции "над клавиатурой", и на
    // возврате из пикера iOS поднимает клавиатуру обратно, потому что поле
    // никто не расфокусировал. Второй тап по скрепке приходится ровно на эту
    // анимацию и пропадает в ней — отсюда "нажал, ничего не происходит".
    //
    // Сняв фокус здесь, мы двигаем клавиатуру, таб-бар и инпут-бар одним
    // переходом, пока пикер уже накрывает экран, и на возврате не двигается
    // ничего: кнопка стоит на месте и ловит следующий тап.
    textInputRef.current?.blur();
  ...
  const renderBubbleContent = (message: ChatMessage, isMine: boolean, isMediaBubble: boolean, readers: ChatReadState[]) => {
    ...
    // Куда садится время. Обычно — в конец последней строки текста (или своей
    // строкой у голосовых), но как только у сообщения появились реакции, под
    // текстом вырастает строка чипсов, и время оказывалось висеть НАД ней,
    // посреди пузыря. Тогда оно переезжает в правый нижний угол самого пузыря
    // и садится на строку реакций справа от чипсов — как в Telegram.
    //
    // Медиа тут ни при чём: у фото/видео время лежит пилюлей на самом кадре
    // (chat-bubble-time--overlay), а строка реакций идёт ПОД кадром, то есть
    // никогда с ним не спорит — и переезжать ему некуда и незачем.
    //
    // Условие намеренно по СПИСКУ реакций, а не по тому, есть ли ещё чипсы в
    // DOM: снятие последней реакции переставляет время обратно в текст сразу,
    // не дожидаясь, пока уходящий чипс доиграет свои 160 мс и схлопнется
    // строка. Ждать было бы нечем — renderBubbleContent не компонент, своего
    // состояния у него нет, а заводить его ради одного кадра на действии,
    // которое делает только сам автор реакции и только когда он в ней один,
    // дороже, чем сам эффект.
    const timeInCorner = !isMediaBubble && reactions.length > 0;
    ...
    <>
      ...
      {message.text && (
        <div className="chat-bubble-text">
          ...
          {/* Время текста — не своей строкой снизу, а "утоплено" в правый
              ...
              Обе половинки — и распорка, и абсолют — снимаются, как только у
              сообщения появились реакции (timeInCorner): утапливать время в
              текст больше некуда, оно уезжает в угол пузыря на строку чипсов,
              и держать под него место в конце последней строки значило бы
              оставить там дыру. */}
          {!isMediaBubble && !timeInCorner && (
            <>
      ...
      {!isMediaBubble && !message.text && !timeInCorner && (
      ...
      <ReactionsRow>
        ...
        {/* Место под время на последней строке чипсов — пустой двойник самой
            метки (см. .chat-reactions-time-slot). Само время лежит абсолютом
            снаружи клипа: внутри его срезало бы overflow'ом на всё время
            анимации высоты. */}
        {timeInCorner && (
          <span className="chat-reactions-time-slot" aria-hidden="true">{timeLabel}</span>
        )}
      </ReactionsRow>
      {/* Время, вытесненное реакциями из текста, — в правом нижнем углу
          пузыря. Соседом клипа, а не внутри него: клип на время анимации
          режет всё, что в него не влезло, а время не появляется вместе со
          строкой, оно просто едет вниз вместе с растущей кромкой пузыря. */}
      {timeInCorner && (
        <span className="chat-bubble-time chat-bubble-time--corner">{timeLabel}</span>
      )}
  ...
  //
  // Кружок — метка ПОЗИЦИИ, а не расписка о конкретном сообщении: он висит под
  // кромкой строки и читается как "дочитал досюда", то есть автоматически и
  // про всё, что выше (ровно как полоса "непрочитанные" про своё чтение).
  // Пофактный вопрос "кто прочитал именно это сообщение" отвечает карточка по
  // тапу, и разводить эти два состояния — смысл всей затеи (см. 7d38ff3).
  //
  // Отсюда правило: метка показывает только то, чего не доказывает собственное
  // сообщение читателя. Если ниже его кружка висит его же ответ — кружок
  // сообщает ноль (написал = дочитал), а места и внимания занимает столько же.
  // Поэтому, наткнувшись при поиске цели на сообщение самого читателя, мы не
  // отступаем к ближайшему чужому, а не рисуем метку вовсе. Так кружок
  // остаётся ровно в том состоянии, ради которого он есть: "прочитал и
  // промолчал". Заодно это снимает и вопрос "почему у меня метки на чужих
  // сообщениях" — в живом разговоре их там больше нет.
  //
  // Формально это разворот d1e07a0 ("кружок больше не пропадает после ответа
  // собеседника"): пропадание было записано там как баг, но по смыслу оно
  // правильное. Кривой в d1e07a0 была реализация (отказ вместо отступа),
  // и правилась она вслепую, без этого правила.
  const readMarkers = useMemo(() => {
    ...
    for (const r of reads) {
      ...
      for (let i = messages.length - 1; i >= 0; i -= 1) {
        ...
        // Первым с конца встретилось собственное сообщение читателя — значит
        // он уже ответил, и кружка здесь быть не должно (см. большой коммент
        // выше). Раньше мы в этом месте отступали к ближайшему чужому
        // сообщению и вешали метку туда — отсюда и брались кружки на строках,
        // под которыми и так висит ответ того же человека.
        if (m.user_id === r.user_id) break;
        anchor = m;
        break;
      }
      // Кружка нет: либо читатель ответил, либо прочитанное вымылось из
      // загруженной истории и вешать метку просто не на что.
      if (!anchor) continue;
        ...
        <div className="chat-input-row" ref={inputRowRef}>
          {!recording && (
            <button
              ...
              disabled={sending}
              // Фокус тут, в отличие от кнопок отправки и микрофона ниже,

...
// State machine:
//   IDLE      - camera rests, uploadTask may run
...
// Initialized once in setup(). Never deinit -- avoids reinit latency and instability.
void initCamera() {
    camera_config_t cfg = {};
    ...
    if (psramFound()) {
        ...
        cfg.jpeg_quality = 10;
        cfg.fb_count     = 3;                  // 3 buffers: capture/queue/consume overlap
        cfg.grab_mode    = CAMERA_GRAB_LATEST; // always serve freshest frame
        ...
        case WStype_TEXT: {
            ...
            if (cmd == "AUTH_OK") {
                ...
                if (part.startsWith("start")) {
                    ...
                    // Switch to HD on-the-fly -- no camera deinit needed
                    sensor_t* s = esp_camera_sensor_get();

...
class BackgroundWorker:
    ...
    async def _notify_house_power(self, offline: bool):
        ...
        Без явного tag оба события падают на дефолт service worker'а
        ('chat-message', см. sw.js) — тот же тег, что у обычных сообщений
        чата. Тег решает, какие уведомления схлопываются в одно: с общим
        дефолтом "дом обесточен" и "связь восстановлена" стирали друг друга
        в шторке, а заодно и любое пришедшее между ними сообщение чата
        стирало "обесточен" ещё до того, как его увидели. Отдельные теги на
